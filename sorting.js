...
function extractBaseDomain(domain) {
  ...
  if (hadLeadingWildcard && normalized) {
    ...
    logDebug(
      `Checking *.pattern: original="${originalPattern}", normalized="${normalized}", parts=${parts.length}, isTldOnly check...`,
    );

    // Check if it's just a TLD (single part) or a multi-part TLD
    const isTldOnly = parts.length === 1 || (parts.length === 2 && multiPartTlds.includes(normalized));
...
function checkStructuralCoverage(pattern1, pattern2) {
  ...
  while (p1Pos < p1Parts.length && p2Pos < p2Parts.length) {
    if (p1Parts[p1Pos] === '*') {
      p1Pos++;
      while (p2Pos < p2Parts.length && p1Pos < p1Parts.length && p2Parts[p2Pos] !== p1Parts[p1Pos]) {
        p2Pos++;
      }
  ...
  while (globalChanged && iterations < maxIterations) {
    ...
    for (const pair of pairs) {
      ...
      if (pair.position1 < pair.position2 && patternCovers(pair.pattern1, pair.pattern2)) {
        ...
        logDebug(
          `Moving "${pair.pattern1}" (pos ${pair.position1}) after "${pair.pattern2}" (pos ${pair.position2}) to pos ${newPos1}`,
        );
      }
    }
...
function sortRules(rulesText) {
  try {
    ...
    for (const line of lines) {
      ...
      const containerName = trimmed.slice(commaIndex + 1).trim();
    ...
    const parsedRules = validRules.map((line, originalIndex) => {
      ...
      const pattern = line.slice(0, commaIndex).trim();
      const containerName = line.slice(commaIndex + 1).trim();
      let domain;
