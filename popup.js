...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('saveRule').addEventListener('click', async () => {
    ...
    const isDomainPattern =
      /^(\*\.)?([a-zA-Z0-9_-]+\.)*([a-zA-Z0-9_*-]+)(\.[a-zA-Z0-9_-]+)*(\.\*)?(\/.*)?$/.test(pattern) || // e.g., google.com, *.google.*, google.*, *.google.com, google.com/search
      pattern.includes('/'); // e.g., google.com/search, google.com/?olud

    if (!isDomainPattern) {
      showMessage(
        'Pattern must be a valid domain (e.g., google.com, *.google.*, google.*, *.google.com) or URL path (e.g., google.com/search)',
        'error',
      );
      return;
    }
    ...
    try {
      ...
      if (DEBUG) {
        const { rules: savedRules, containerStyles: savedStyles } = await browser.storage.local.get([
          'rules',
          'containerStyles',
        ]);
      ...
      if (convertTempContainer && targetContainerId) {
        try {
          ...
          logDebug(
            `Renamed container ${targetContainerId} to ${containerName} with color ${selectedColor} and icon ${selectedIcon}`,
          );
  ...
  async function saveRuleEdits() {
    ...
    try {
      ...
      const currentContainer =
        currentTab.cookieStoreId && currentTab.cookieStoreId !== 'firefox-default'
